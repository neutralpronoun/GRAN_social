...
class DataPrep():
    ...
    def prep_data(self, index, use_cuda = False):
        ...
        nodelist = list(self.graph1.nodes)
        num_features = self.graph1.nodes[nodelist[0]]["feature"].shape[0]

        y = np.zeros((self.num_nodes, num_features), dtype=np.float32)
        for i, n in enumerate(nodelist):
            y[i, :] = self.graph1.nodes[n]["feature"]
            # print(self.graph1.nodes[n]["feature"])
            #print(self.graph1.nodes[n])
        # print(y)
...
class NodeFeatPredict(torch.nn.Module):
    def __init__(self, hidden_channels, num_prop, dataset):
        ...
        self.conv1 = GCNConv(dataset.input_dim, hidden_channels)
        self.conv2 = GCNConv(hidden_channels, dataset.num_features)

        self.conv_inter = GCNConv(hidden_channels, hidden_channels)
        self.lin = nn.Linear(dataset.num_features, dataset.num_features)
...
def diagnostic_plot(G, pred, true, show = True, index = 1):
    fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(ncols=2, nrows=2, figsize=(16, 16))
    ...
    cmin = np.min([np.array(true)[:,0], np.array(pred)[:,0]])
    cmax = np.max([np.array(true)[:, 0], np.array(pred)[:, 0]])
    ...
    ax3.scatter(np.array(true)[:,0], np.array(pred)[:,1])
    ax4.scatter(np.array(pred)[:, 0], np.array(pred)[:, 1])
    ...
    plt.savefig(f"graph_example_{index}_feat.jpg")
    if show:
        plt.show()
...
model = NodeFeatPredict(hidden_channels = 20, num_prop=0, dataset = data)
print(model)
